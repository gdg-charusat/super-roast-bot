...
def test_prompt():
    ...
    assert "NEVER roast" not in SYSTEM_PROMPT, "[ERROR] 'NEVER roast' still present"
    print("[PASS] No contradictory 'NEVER roast' rule")
...
def test_rag():
    ...
    try:
        ...
        assert isinstance(result, str) and len(result) > 0, "[ERROR] RAG returned invalid result"
        print(f"[PASS] RAG fallback works: '{result[:50]}'")
    except ImportError as e:
        print(f"[WARN] SKIPPED: FAISS not available on this system ({e})")
